...
// Crop is also carried inside ImageTool.tsx (Image Resizer, which crops as
// part of its downsize flow) and duplicated again in the SPA's
// src/features/croptool/CropPage.tsx — this app deliberately does not
// import from src/web/spa (see the note atop api.ts), so the small
...
const CropTool: React.FC<Props> = ({ onBack }) => {
  ...
  return (
    <section className="tool-view">
      ...
      <h1 className="tool-heading">Crop Image</h1>
      <p className="muted local-notice">Processed locally — images never leave your browser.</p>

      <div className="imgtool-field">
        <label htmlFor="croptool-file">Choose an image</label>
        <input id="croptool-file" type="file" accept="image/*" onChange={handleFileChange} />
      </div>
      ...
      {meta && (
        <>
          ...
          <div className="imgtool-controls">
            <div className="imgtool-field">
              <label htmlFor="croptool-format">Output format</label>
              <select id="croptool-format" value={format} onChange={(e) => setFormat(e.target.value as Format)}>
                <option value="image/webp">WebP</option>
                <option value="image/jpeg">JPEG</option>
              </select>
            </div>

            <button type="button" className="btn btn-primary" onClick={handleCrop} disabled={processing || !hasSelection}>
