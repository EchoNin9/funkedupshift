...
export function AdminDashboard() {
  ...
  if (!user || visibleModules.length === 0) {
    return (
      <div className="container-max section-padding text-center">
        <h1 className="text-2xl font-sans font-bold text-text-primary mb-4">
          Access Denied
        </h1>
  ...
  return (
    <main className="container-max section-padding">
      <motion.h1
        className="text-4xl sm:text-5xl font-sans font-bold text-gradient mb-10"
      ...
      >
        {visibleModules.map((mod) => (
          <motion.div key={mod.path} variants={fadeUp}>
            <div className="card p-6 hover:border-accent-500/50 transition-colors group">
              <Link to={mod.path} className="block">
                <mod.icon className="w-8 h-8 text-accent-500 mb-4 group-hover:scale-110 transition-transform" />
                <h2 className="text-lg font-sans font-bold text-text-primary mb-1">
                  {mod.label}
                </h2>
              ...
              {(mod.id === "websites" || mod.id === "media") && (
                <Link
                  ...
                  className="mt-4 inline-flex items-center gap-1.5 text-sm font-medium text-accent-400 hover:text-accent-300 transition-colors"
                >
