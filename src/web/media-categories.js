(function () {
  ...
  function attachCategoryHandlers() {
    ...
    Array.prototype.forEach.call(document.querySelectorAll('.delete-cat'), function (btn) {
      btn.addEventListener('click', function () {
        ...
        fetchWithAuth(base + '/media-categories?id=' + encodeURIComponent(id), { method: 'DELETE' })
          .then(function (r) {
            if (r.ok) return loadCategories();
            return r.text().then(function (t) { throw new Error(t || 'Failed'); });
          })
  ...
  function renderCategoryPage() {
    ...
    if (total === 0) {
      categoryList.innerHTML = '<li>No media categories yet.</li>';
    ...
    fetchWithAuth(base + '/media-categories')
      ...
      .then(function (data) {
        ...
        if (typeof localStorage !== 'undefined') {
          try {
            var cacheKey = 'funkedupshift_media_categories';
            var list = (allCategoriesData || []).map(function (c) {
              return { id: c.PK || c.id || '', name: c.name || c.PK || c.id || '' };
            });
            localStorage.setItem(cacheKey, JSON.stringify(list));
          } catch (e) {
            console.error('Failed to save media categories to cache:', e);
          }
        }
        renderCategoryPage();
      })
